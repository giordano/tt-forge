...
from third_party.tt_forge_models.segformer.semantic_segmentation.pytorch.loader import (
    ModelLoader as SegformerLoader,
    ModelVariant as SegformerVariant,
)
from .utils import (
...
def test_segformer_torch_xla(
    training, batch_size, input_size, channel_size, loop_count, task, data_format, model_name, measure_cpu
):
    """
    This function creates a Segformer model using PyTorch and torch-xla.
    ...
    model_variant = SegformerVariant.B0_FINETUNED
    segformer_loader = SegformerLoader(model_variant)
    model_info = segformer_loader.get_model_info(model_variant).name
    ...
    full_model_name = "Segformer Torch-XLA"
    ...
    print_benchmark_results(
        model_title="Segformer Torch-XLA",
...
def benchmark(config: dict):
    """
    Run the segformer torch-xla benchmark.
    ...
    return test_segformer_torch_xla(
        training=training,
        batch_size=batch_size,
        input_size=input_size,
        channel_size=channel_size,
